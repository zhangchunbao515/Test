//顺序栈的物理实现方法，其中包含一个溢出处理函数，此函数为类的私有成员,在主函数中完成中缀转后缀的操作,并且实现计算器类
#include <iostream>
#include <string>
using namespace std;
...
void main()
{
	...
	//计算器类的实现代码，用于实现简单的四则运算，目标代码为后缀表达式，用postfix[maxsize]存储目标代码。
	int Size;
	cout<<"请输入需要创建栈预计最大长度（此栈用于存放后缀表达式的数字量）：";
	cin>>Size;
	Stack <int> s1(Size);//建立一个操作数栈，用与后缀表达式在扫描时操作数进栈
	...
	for(int k=0;k<j;k++)
	{
		...
		if (flag)
		{
			s1.Push(temp);//判断如果后缀表达式的某个字符是操作数直接进栈，进入下一个循环
			cout<<"push:";
			s1.print();
		}
		else
		{
			...
			switch(postfix[k])
			{
				case'+': 
					value = (left) + (right);
					s1.Push(value);
					s1.getTop(temp);/*获得栈顶,发现再次让其出栈时栈顶元素值错误，下同*/
					cout<<"栈顶元素为："<<temp;
					cout<<endl; break;
				case'-': 
					value = (left) - (right);
				...
				case'*': 
					...
					cout<<endl;  break;
				case'/':
					value = (left) / (right);s1.Push(value);s1.getTop(temp);cout<<"栈顶元素为："<<temp;cout<<endl; break;
			}
			cout<<endl<<"PUSH:";
			s1.print();
		}
